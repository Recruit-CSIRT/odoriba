...
# [customized] this module is to handle customized http base cuckoo agent.
#              additional function is tab # custom comments
...
import internet_control,subprocess# custom
    ...
    def wait_for_completion(self):
        ...
        while db.guest_get_status(self.task_id) == "running":
            ...
            if status == CUCKOO_GUEST_COMPLETED or FIN_flag==True: #custom
                ...
                log.debug("%s: analysis not completed yet (status=%s) (spent=%ds/%s)",
                          self.id, status, s, self.timeout)  # custom
                ############### custom from here #################
                if burn == False: 
                    s,FIN_flag = internet_control.check(self.hv,self.timeout,log,int(self.burn_limit))
                    if FIN_flag != True: burn = True; log.debug("[return odoriba] internet control out.. %ds"%s)
